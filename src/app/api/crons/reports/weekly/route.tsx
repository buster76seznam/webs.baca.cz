...
export async function GET(request: Request) {
  ...
  // Calculate date range for the last week (Monday to Sunday)
  ...
  const from = lastMonday.toISOString();
  const to = lastSunday.toISOString();
  ...
  try {
    ...
    const newInfluencers = (influencers as Partner[]).filter(i => {
        ...
        return createdAt >= lastMonday && createdAt <= lastSunday;
    }).length;
    ...
    return NextResponse.json({ message: 'Error generating weekly report.' }, { status: 500 });
  }
}
