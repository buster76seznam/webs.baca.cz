...
export default function OnlineUsersBar({ currentUserId }: OnlineUsersBarProps) {
  ...
  return (
    <div className="mb-8 p-5 rounded-none bg-white/[0.02] border border-white/5">
      ...
      <div className="flex flex-wrap gap-2">
        {online.length === 0 ? (
          ...
          online.map(u => (
            <span
              key={u.id}
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-none text-[10px] font-black uppercase tracking-wider border
                ${u.id === currentUserId ? 'border-[#7C3AED]/40 bg-[#7C3AED]/10 text-[#A78BFA]' : 'border-white/10 bg-white/[0.03] text-zinc-300'}`}
            >
              <span className="w-1.5 h-1.5 rounded-none bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
              {u.username}
